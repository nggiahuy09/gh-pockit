...
class AccountMapper {
  ...
  /// Parses a row. Any failure is a [GPDatabaseFailure], never the more specific thing that went wrong.
  ///
  /// That flattening is deliberate. A blank `name` in a *row* is not the same event as a blank name in a text field: the user is not typing, they are
  /// looking at a list, and telling them "Account name can't be empty" would be a sentence about an input that is not on screen. What actually happened is
  /// that local data cannot be read — which is what [GPDatabaseFailure] says, and which is loud on purpose, because golden rule 1 leaves no remote copy to
  /// fall back to.
  ///
  ...
  GPResult<AccountEntity> toEntity(AccountRow row) {
    ...
    if (type == null) {
      ...
      return const GPErr<AccountEntity>(GPDatabaseFailure());
    }
    ...
    return switch (entity) {
      GPOk<AccountEntity>() => entity,
      // Re-labelled, not forwarded: see the method doc on why a row's validation failure is a database failure.
      GPErr<AccountEntity>() => const GPErr<AccountEntity>(GPDatabaseFailure()),
    };
  }
