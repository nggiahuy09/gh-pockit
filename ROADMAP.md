...
| Flex | ADR-0001 _Use Drift as local source of truth_. Repository test với in-memory DB                                                                                 | 🟡 ADR-0001 draft xong, cả 3 open question đã chốt — còn _Accepted_ khi test `create → watchAccounts emit` pass (T4/T6)                                                                                                                                                |
| Flex | Nợ từ W1: bảng `settings` local-only + `GPDriftLocaleStore` thay `GPInMemoryLocaleStore` (ADR-0004). Chưa làm = ngôn ngữ user chọn không sống qua restart       | 🟡 Bảng `settings` xong ở T2; `GPDriftLocaleStore` **chưa** — `injector.dart` vẫn bind `GPInMemoryLocaleStore`                                                                                                                                                         |
