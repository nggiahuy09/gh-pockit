...
| T4   | `AccountDao`: insert / update / `watchAccounts()` / `archive()`                                                                                                 | ✅ Done — `features/accounts/data/daos/`, 11 test. `watchAccounts()` **ẩn archived mặc định** (`includeArchived: false`) nên có thêm `unarchive()`, không thì row bị kẹt. `update`/`archive` đẩy `updated_at` nhưng **không** đụng `version` (server sở hữu — §7). DAO giữ `GPClock`, không nhận timestamp từ caller |
...
| Flex | ADR-0001 _Use Drift as local source of truth_. Repository test với in-memory DB                                                                                 | ✅ ADR-0001 **Accepted** (T4) — `create → watchAccounts emit` pass. Repository test thật vẫn chờ T6; DAO test đã chạy trên in-memory DB                                                                                                                                                                              |
| Flex | Nợ từ W1: bảng `settings` local-only + `GPDriftLocaleStore` thay `GPInMemoryLocaleStore` (ADR-0004). Chưa làm = ngôn ngữ user chọn không sống qua restart       | ✅ Done (T4) — `core/localization/drift_locale_store.dart`, 6 test, đổi đúng 1 dòng trong `injector.dart` như ADR-0004 hứa. Code lạ đọc ra `null` (= theo máy) chứ không fallback `en`. Đánh đổi: `bootstrap()` giờ mở DB trước frame đầu                                                                            |
